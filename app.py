...
def load_demo_test():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'Demo test dataset predicted', 'total_rows': len(df_test_original), 'prediction_summary': df_test_original['predicted_disposition'].value_counts().to_dict()})
    except Exception as e:
...
def clear_data():
    ...
    file_data.clear()
    model_data.clear()
    return jsonify({'success': True, 'message': 'All data cleared from memory'})
